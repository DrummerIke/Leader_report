...
Рабочая форма ежемесячного отчёта лидеров с кейсами, факторами, премиями/штрафами и выгрузкой XLSX для администратора. Для работы требуется подключение к Supabase; демонстрационные данные и локальное сохранение не используются.
